...
### 21. ~~Mobile responsive polish~~ **Done**
Audited and fixed all pages. Changes: `PageContainer` padding responsive (`px-4 sm:px-6 md:px-8`); Navbar gains hamburger menu with slide-down mobile drawer (hidden on `sm:`+); HomePage hero text scales down (`text-2xl md:text-4xl`), category tabs horizontally scrollable on mobile, filter row below tabs; `AuctionDetailPage` image height responsive (`h-48 sm:h-64 md:h-80 lg:h-[400px]`), sidebar sticky only on `lg:`+; `BiddingPanel` padding `p-4 md:p-8`; all stat card rows changed from `flex` to `grid grid-cols-2 md:grid-cols-4`; seller/shop header rows stack (`flex-col sm:flex-row`) on narrow screens; `SellerAuctionDetailPage` bid history table wrapped in `overflow-x-auto` with `min-w-[480px]` inner div; `ProfilePage` sidebar stacks above main content on mobile with horizontal tab scroll; `ShopDetailPage` + `PaymentPage` card padding responsive.
