...
function toAuction(b: BackendAuction): Auction {
  const pickupStart = b.pickup_start ? new Date(b.pickup_start).getTime() : undefined
  const pickupEnd   = b.pickup_end   ? new Date(b.pickup_end).getTime()   : undefined
  return {
    auction_id:          b.auction_id,
    item: {
      title:     b.item_title  ?? '',
    ...
    pickup_start:        pickupStart && pickupStart > 0 ? pickupStart : undefined,
    pickup_end:          pickupEnd && pickupEnd > 0     ? pickupEnd   : undefined,
  }
}
